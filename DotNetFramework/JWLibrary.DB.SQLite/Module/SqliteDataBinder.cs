...
namespace GSCoreLibray.DB.MsSql.Module {
    ...
    public class SqliteDataBinder<T> : IDataBinder<T> {
        private T _instance;
        private IDataReader _reader;

        public SqliteDataBinder(Type type, IDataReader reader) {
        ...
        public T DataBind() {
            SqliteDataBinderAttribute binderAttr = null;
            ...
            try {
                foreach (FieldInfo field in _instance.GetType().GetFields()) {
                    customAttr = null;
                    binderAttr = null;
                    customAttr = field.GetCustomAttributes(typeof(SqliteDataBinderAttribute), false);

                    if (customAttr != null && customAttr.Length > 0)
                        binderAttr = customAttr[0] as SqliteDataBinderAttribute;
                ...
                foreach (PropertyInfo property in _instance.GetType().GetProperties()) {
                    customAttr = null;
                    binderAttr = null;
                    customAttr = property.GetCustomAttributes(typeof(SqliteDataBinderAttribute), false);
