...
namespace JWLibrary.Version {
    ...
    public class ConfigManager {
        ...
        public ConfigInfo GetInitialConfig(string ovnerName, Dictionary<string, string> etcs = null) {
            ...
            var result = LiteDbFlexerManager.Instance.Value.Create<ConfigInfo>().EnsureIndex(x => x.OwnerName)
                .Get(x => x.OwnerName == ovnerName).GetResult<ConfigInfo>();
            if (result.jIsNull()) {
        ...
        public ConfigInfo GetConfig(string ownerName) {
            return LiteDbFlexerManager.Instance.Value.Create<ConfigInfo>().EnsureIndex(x => x.OwnerName)
                .Get(x => x.OwnerName == ownerName).GetResult<ConfigInfo>();
        }
        ...
        public class ConfigInfo {
            ...
            public int OwnerNowVersion {
                get => OwnerOldVersion + 1;
                set => value = value + OwnerOldVersion;
            }

            public int OwnerOldVersion { get; set; }
